...
export const Transaction = ({ rawTX }: any) => {
  ...
  const getTx = async () => {
    ...
    try {
      ...
      const completedMultisigTransaction = alltransaction.results.filter(item => item.txType === 'MULTISIG_TRANSACTION' && item.isExecuted === true);

      const queuedMultisigTransaction = queuedtransaction.results.filter(item => item.isExecuted === false);
      setCompletedTransaction(completedMultisigTransaction)
  ...
  const handleSignTx = async () => {
    ...
    let transactionSafe1_1 = await protocol.createTransaction({
     ...
     const transaction = await protocol.signTransaction(transactionSafe1_1, SigningMethod.ETH_SIGN_TYPED_DATA_V4);
