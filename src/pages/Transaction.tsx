        ...
        transactions?.map((transaction: any, index: number) => (
          <TableRow key={index} className="w-full">
                ...
                >
                  ...
                  <AccordionContent>
                    {transaction.confirmations.map(
                      (confirmation: SafeMessageConfirmation, idx: number) => (
                        <div
                          key={idx}
                          className="flex gap-5 justify-between p-2 border-b border-gray-800"
                        >
                          <div>{convertToSlice(confirmation.owner)}</div>
                          <Checkbox
                            checked={confirmation.signature ? true : false}
                            disabled
                          />
                        </div>
                      )
            ...
            <TableCell className="text-center">
              ...
              <button
                onClick={openModal}
                className="border border-gray-700 p-2 hover:bg-neutral-900 hover:bg-opacity-50 rounded-md "
              >
                View
              </button>
            </TableCell>
...
export const Transaction = () => {
  const {
    ...
    safe,
    ethAdapter,
  }: { clientSigner: JsonRpcSigner; safe: Safe; ethAdapter: EthersAdapter } =
    useEthereum();
  ...
  const getTx = async () => {
    try {
      ...
      const queuedMultisigTransaction = queuedtransaction.results.filter(
        (item) =>
          item.isExecuted === false && item.origin === SAFE_TRANSACTION_ORIGIN
      );
  ...
  const handleSignTx = async () => {
    ...
    try {
      ...
      if (
        newPendingTransaction.confirmationsRequired ===
        newPendingTransaction.confirmations.length
      ) {
  ...
  return (
    <div className="w-full p-4 flex justify-center items-center gap-5">
      <PremiumButton onClick={() => handleSignTx()} label="Sign" loading={isSignButtonLoading} />
      <Button variant="destructive" onClick={() => handleRejectTx()}>
