...
const GlobalStateProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const handleConfirmTX = async (safeTxHash: string) => {
        try {
            ...
            const executeTransaction = safe && safeTransaction && await safe.executeTransaction(safeTransaction);
        ...
        try {
          ...
          if (pendingTxData && pendingTxData.pending) {
            if (isRejected) {
              if (pendingTxData.rejected) {
                ...
                signTransaction = safe && await safe.createRejectionTransaction(
            ...
            // here
            const nextNonce = !isNewTransaction ? (pendingTransactions && pendingTransactions.pending?.nonce) : (safeApiKit && await safeApiKit.getNextNonce(multiSigAddress));
    
            console.log(nextNonce, pendingTransactions?.pending?.nonce, safeApiKit && await safeApiKit.getNextNonce(multiSigAddress))
    
            signTransaction = safe && await safe.createTransaction({
              transactions: [
                {
          ...
          if (signTransaction) {
            ...
            if (!isProposed) {
              ...
              safeApiKit && clientSigner && senderSignature && await safeApiKit.proposeTransaction({
              ...
              window.location.reload();
                ...
                safeApiKit && senderSignature &&  await safeApiKit.confirmTransaction(safeTxHash, senderSignature.data);
            }
          } else {
